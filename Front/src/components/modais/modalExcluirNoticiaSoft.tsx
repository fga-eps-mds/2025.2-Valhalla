...
interface ModalExcluirNoticiaSoftProps {
  isOpen: boolean;
  onClose: () => void;
...
export default function ModalExcluirNoticiaSoft({ isOpen, onClose, noticiaId, onDeleted }: ModalExcluirNoticiaSoftProps) {
const [isDeleting, setIsDeleting] = useState(false);


  const handleDeleteNoticia = async (idParaDeletar?: number) => {
    const id = idParaDeletar ?? noticiaId;
    if (!id) {
      toast.error('ID da notícia inválido.');
      return;
    }
    setIsDeleting(true);
    try {
      ...
      toast.error('Erro ao tentar deletar a notícia.');
    } finally {
      setIsDeleting(false);
    }
  };
